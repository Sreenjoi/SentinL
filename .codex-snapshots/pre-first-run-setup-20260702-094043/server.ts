...
export async function createApp() {
  ...
  app.get("/api/discord/permissions/:serverId", requireAuth, requireServerAuth, async (req: any, res: any, next: any) => {
    try {
      ...
      const permissions = {
        ...
        KickMembers: botMember.permissions.has("KickMembers"),
        BanMembers: botMember.permissions.has("BanMembers"),
        ManageMessages: botMember.permissions.has("ManageMessages"),
        ModerateMembers: botMember.permissions.has("ModerateMembers"),
        ReadMessageHistory: botMember.permissions.has("ReadMessageHistory"),
        AddReactions: botMember.permissions.has("AddReactions"),
        AttachFiles: botMember.permissions.has("AttachFiles"),
        EmbedLinks: botMember.permissions.has("EmbedLinks")
      };

      res.json({ permissions, raw: botMember.permissions.toArray() });
    } catch (e: any) {
      next(e);
    }
  });
    ...
    try {
      ...
      if (recData.feedbackIds && Array.isArray(recData.feedbackIds)) {
        ...
        for (const fId of recData.feedbackIds) {
          batch.delete(db.collection("trainingFeedback").doc(fId));
        }
        await batch.commit();
        invalidateTrainingCache(body.serverId);
      }
  ...
  app.get(
    ...
    async (req, res, next: any) => {
               ...
               if (window.opener) {
                window.opener.postMessage({ 
                  ...
                  payload: {
                     ...
                     avatar: ${JSON.stringify(discordUser.avatar)},
      ...
      `;
        ...
        res.status(500).json({ error: "Account linking failed. Please try again." });
      }
    },
  );
  ...
  app.post("/api/guilds/:serverId/activation", requireAuth, requireServerAuth, async (req: any, res, next: any) => {
    try {
      ...
      if (active) {
        updateData.ownerEmail = (req as any).user.email;
      } else {
