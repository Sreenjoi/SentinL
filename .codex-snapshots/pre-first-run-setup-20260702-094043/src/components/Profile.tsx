...
export default function Profile() {
  ...
  const { discordProfile, tier, userTier, isTrial, userIsTrial, isBetaTester } = useServer();
  ...
  return (
    ...
    >
      <div className="mb-10 sm:mb-12 flex flex-col md:flex-row items-center text-center md:text-left gap-4 sm:gap-6">
        <div className="w-24 h-24 sm:w-28 sm:h-28 bg-white/60 backdrop-blur-xl border-4 border-white shadow-xl shadow-primary/10 rounded-full flex shrink-0 items-center justify-center text-4xl sm:text-5xl text-primary font-black md:mb-0 relative group overflow-hidden">
          {discordProfile?.avatar ? (
            ...
            <>
              <div className="absolute inset-0 bg-primary/10 group-hover:bg-primary/20 transition-colors duration-300 ease-out"></div>
              {user.email?.[0].toUpperCase()}
            </>
          )}
        </div>
        <div className="min-w-0">
          <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-on-surface mb-1 truncate">
            {displayName || user?.displayName || "User Profile"}
          </h1>
          <p className="text-[9px] sm:text-[11px] font-black uppercase tracking-[0.2em] text-text-secondary">
