...
export default function CustomCommandsBuilder() {
  ...
  const syncCommands = async () => {
    ...
    try {
      ...
      if (data.success) {
        setSyncMessage("Synchronized with Discord Successfully!");
    ...
    <>
      <div className={`flex flex-col gap-8`}>
        ...
        <ProGate isPro={isPro} featureName="Custom Commands" featureDescription="Build powerful custom slash commands for your server" className="w-full">
          <div className="flex flex-col lg:flex-row gap-8 transition-all duration-300 ease-in-out w-full">
            {/* Sidebar */}
      <div className="w-full lg:w-1/3 flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-3">
              <h2 className="text-xl font-black text-on-surface">Commands</h2>
              {!isPro && (
                <span className="text-[9px] bg-primary/10 text-primary px-1.5 py-0.5 rounded font-bold uppercase tracking-widest">
                  PRO
                </span>
              )}
            </div>
            <p className="text-xs text-text-secondary">
              Your bespoke commands
            </p>
          </div>
          <button
              onClick={handleCreate}
              disabled={!isPro}
              className="w-10 h-10 rounded-2xl bg-primary text-white flex items-center justify-center hover:scale-105 active:scale-95 transition-all shadow-lg shadow-primary/20 disabled:opacity-50"
            >
              <Plus className="w-5 h-5" />
            </button>
          </div>

          <div className="bg-white/80 backdrop-blur-md rounded-3xl border border-white/40 shadow-xl shadow-primary/5 p-4 flex flex-col gap-2 min-h-[400px]">
            {loading ? (
              ...
              <CompactEmptyState 
                title="No commands created yet."
                description="Click + to start compiling one."
              />
            ) : (
              commands.map((cmd) => (
                <div
                  key={cmd.id}
                  className={`w-full rounded-2xl transition-all duration-300 group flex items-center pr-2 ${
                    selectedId === cmd.id
                      ? "bg-primary text-white shadow-xl shadow-primary/20"
                      : "bg-surface-container/30 text-on-surface hover:bg-surface-container/80"
                  }`}
                >
                  <button
        ...
        <div className="w-full lg:w-2/3 flex flex-col gap-6">
          <div className="flex items-center justify-between pb-2 border-b border-outline-variant/10">
            <div>
              <h2 className="text-xl font-black text-on-surface flex items-center">
                Command Builder
                <Tooltip text="Once you configure your commands here, you must push them to Discord for them to become active in your server." />
              </h2>
              <p className="text-xs text-text-secondary">
                Chain actions together iteratively.
              </p>
            </div>
            <div className="flex items-center gap-3">
              {syncMessage && (
              ...
              <button
                ...
                className="px-4 py-2 bg-secondary text-on-surface rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-secondary/80 transition-all flex items-center gap-2 relative overflow-hidden group disabled:opacity-50"
              >
                <RefreshCw
              ...
              <motion.button animate={isSaved ? { scale: [1, 1.05, 1], transition: { duration: 0.3 } } : {}} whileTap={hasChanges ? { scale: 0.95 } : undefined}
                ...
                className={`px-6 py-2 rounded-xl font-bold text-xs uppercase tracking-widest transition-all shadow-lg flex items-center gap-2 disabled:opacity-50 disabled:hover:scale-100 ${
                  isSaved
                    ? "bg-emerald-500 text-white shadow-emerald-500/20 hover:bg-emerald-600"
                    : !hasChanges
                      ? "bg-surface-container text-text-secondary/70 shadow-none cursor-default border border-outline-variant/30"
                      : "bg-primary text-white shadow-primary/20 hover:bg-primary/90 hover:scale-105"
          ...
          {!selectedCommand ? (
            <div className="flex-1 flex flex-col items-center justify-center text-text-secondary min-h-[400px]">
              <Settings className="w-12 h-12 mb-4 opacity-50" />
              <p>Select or construct a new command from the roster.</p>
            </div>
          ) : (
            <div className="flex flex-col gap-8">
              {/* Info Fields */}
              <div className="bg-white/80 backdrop-blur-md rounded-3xl border border-white/40 shadow-xl shadow-primary/5 p-8">
                <h3 className="text-xs font-black uppercase tracking-[0.2em] text-text-secondary mb-6 flex items-center gap-2">
                  <AlertCircle className="w-4 h-4" /> Command Trigger Definition
                </h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  <div>
                    <label className="flex text-xs font-bold text-on-surface mb-2 tracking-wide uppercase items-center">
                      Command Name
                      <Tooltip text="The trigger word users will type after a slash. No spaces allowed." />
                    </label>
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full bg-surface-container/30 border border-outline-variant/30 rounded-2xl pl-8 pr-4 py-3 text-sm font-bold text-on-surface focus:outline-none focus:border-primary/50 focus:ring-2 focus:ring-primary/20 transition-all disabled:opacity-50"
                  ...
                  <div>
                    <label className="flex text-xs font-bold text-on-surface mb-2 tracking-wide uppercase items-center">
                      Description
                      <Tooltip text="What users will see when scrolling through the slash commands list." />
                    </label>
                    <input
                      type="text"
                      disabled={!isPro}
                      ...
                      className="w-full bg-surface-container/30 border border-outline-variant/30 rounded-2xl px-4 py-3 text-sm font-medium text-on-surface focus:outline-none focus:border-primary/50 focus:ring-2 focus:ring-primary/20 transition-all disabled:opacity-50"
                  ...
                  <div>
                    <label className="flex text-xs font-bold text-on-surface mb-2 tracking-wide uppercase items-center">
                      Permissions
                      <Tooltip text="Who is allowed to execute this command." />
                    </label>
                    <Select
                      disabled={!isPro}
                  ...
                  <div>
                    <label className="flex text-xs font-bold text-on-surface mb-2 tracking-wide uppercase items-center">
                      Target User
                      <Tooltip text="If YES, the person invoking the command will be prompted to select a target user. Useful for moderating or assigning roles." />
                    </label>
                    <Select
                      disabled={!isPro}
                ...
                <div className="flex flex-col gap-4">
                  <AnimatePresence>
                    {selectedCommand.actions.map((act, index) => (
                      <motion.div
                        ...
                        className="bg-white border border-outline-variant/20 rounded-2xl p-6 relative group"
                      >
                        <div className="flex gap-4">
                          ...
                          <div className="flex flex-col gap-1 items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                            <button
                              ...
                              disabled={!isPro}
                              className="p-1 hover:bg-surface-container rounded-lg text-text-secondary hover:text-on-surface disabled:opacity-50"
                            >
                              <ChevronUp className="w-4 h-4" />
                            </button>
                            <div className="w-6 h-6 flex items-center justify-center rounded-full bg-surface-container text-[10px] font-black">
                              {index + 1}
                            </div>
                            <button
                              onClick={() => moveAction(index, 1)}
                          ...
                          <div className="flex-1 flex flex-col gap-4">
                            <div className="flex justify-between items-center">
                              <div className="px-3 py-1 bg-surface-container/50 rounded-lg text-xs font-black uppercase tracking-wider text-primary border border-outline-variant/20 flex gap-2 items-center">
                            ...
                            {(act.type === "send_message" ||
                              ...
                              <div className="flex flex-col gap-3 mt-2">
                                <label className="flex items-center gap-2 text-sm font-medium text-on-surface">
                                  ...
                                  <Tooltip text="Format this message inside a stylistic Discord embed rather than plain text." />
                                </label>
                                {act.isEmbed && (
                                  <div className="grid grid-cols-2 gap-4">
                                    <input
                                      ...
                                      className="bg-surface-container/30 border border-outline-variant/30 rounded-xl px-4 py-2 text-sm focus:outline-primary/50 disabled:opacity-50"
                                    />
                                    <div className="flex gap-2">
                                      <input
                                        ...
                                        className="w-10 h-10 rounded-lg border-0 bg-transparent cursor-pointer disabled:opacity-50"
                                      />
                                ...
                                <textarea
                                  ...
                                  className="w-full bg-surface-container/30 border border-outline-variant/30 rounded-2xl px-4 py-3 text-sm font-medium text-on-surface focus:outline-primary/50 min-h-[100px] max-h-[300px] resize-none overflow-y-auto disabled:opacity-50 transition-all custom-scrollbar"
                  ...
                  <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <button
                      ...
                      disabled={!isPro}
                      className="flex flex-col text-left px-5 py-4 border border-dashed border-outline-variant/50 hover:border-primary/50 rounded-2xl transition-all duration-300 ease-out bg-white/40 hover:bg-primary/5 hover:shadow-md group disabled:opacity-50"
                    >
                      <div className="text-xs font-black uppercase tracking-widest text-text-secondary group-hover:text-primary transition-colors flex gap-2 items-center mb-1.5">
                        <Plus className="w-4 h-4" /> Send Message
                      </div>
                      <span className="text-[11px] font-medium text-text-secondary/80 leading-relaxed">
                    ...
                    >
                      <div className="text-xs font-black uppercase tracking-widest text-text-secondary group-hover:text-primary transition-colors flex gap-2 items-center mb-1.5">
                        <Plus className="w-4 h-4" /> Remove Role
                      </div>
                      <span className="text-[11px] font-medium text-text-secondary/80 leading-relaxed">
                    ...
                    >
                      <div className="text-xs font-black uppercase tracking-widest text-text-secondary group-hover:text-primary transition-colors flex gap-2 items-center mb-1.5">
                        <Plus className="w-4 h-4" /> Direct Message
                      </div>
                      <span className="text-[11px] font-medium text-text-secondary/80 leading-relaxed">
