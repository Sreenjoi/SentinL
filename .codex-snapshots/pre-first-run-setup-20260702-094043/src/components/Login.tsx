    ...
    const handleRedirect = async () => {
      ...
      try {
        ...
        if (err.code === "auth/email-already-in-use" || err.code === "auth/account-exists-with-different-credential") {
          ...
          errorMessage = "Google sign-in is misconfigured because Firebase Auth was not initialized. Check VITE_FIREBASE_* env vars (or firebase-applet-config.json if in preview).";
        }
        ...
        try {
          ...
          return; // Redirecting...
           ...
           if (String(redirectErr.code).includes("auth/argument-error")) {
             errMsg = "Google sign-in is misconfigured because Firebase Auth was not initialized. Check VITE_FIREBASE_* env vars (or firebase-applet-config.json if in preview).";
           }
           setError(errMsg);
           setIsGoogleLoading(false);
           setLoading(false);
