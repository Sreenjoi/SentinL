    ...
    try {
      ...
      if (data.details) {
        ...
        if (data.details.added.length > 0) {
          toast.success(
            `Added ${data.details.added.length} commands: ${data.details.added.join(", ")}`,
          );
        }
        if (data.details.updated.length > 0) {
          toast.success(
            `Updated ${data.details.updated.length} commands: ${data.details.updated.join(", ")}`,
          );
        }
        if ((data.details.removed || []).length > 0) {
          toast.success(
            `Removed ${data.details.removed.length} duplicate server commands: ${data.details.removed.join(", ")}`,
          );
        }
    ...
    try {
      ...
      if (isCurrentlyActive) {
        ...
        const newIds = Array.from(
          new Set([...activeServerIds, selectedServerId]),
        );
        await setDoc(
          modRef,
          { activeServerIds: newIds, activeServerId: newIds[0] || null },
                ...
                <p className="text-[9px] font-bold text-text-secondary/70 uppercase tracking-wider">
                  ...
                  {isBetaTester && subscription?.expiresAt
                    ...
                    : (userSubscription?.status === "trial" ||
                          subscription?.status === "trial" ||
                          isTrial) &&
                        (userSubscription?.trialEnd || subscription?.trialEnd)
                      ? new Date(
                          (userSubscription?.trialEnd ||
                            subscription?.trialEnd) as string,
                        ).toLocaleDateString()
                      : new Date(
                          (userSubscription?.expiresAt ||
                            subscription?.expiresAt) as string,
                        ).toLocaleDateString()}
                </p>
              </div>
            ) : null}
        ...
        <div className="flex items-center gap-4 sm:gap-6">
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              >
                <RefreshCw
                  className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${syncingBot ? "animate-spin" : ""}`}
                />
              </button>
            </div>
            ...
            {activationError && (
              <div className="absolute top-12 left-1/2 -translate-x-1/2 w-48 bg-danger text-white text-[10px] p-2 rounded-lg text-center z-50 shadow-xl flex items-center gap-2">
                ...
                <button
                  ...
                  className="ml-auto hover:opacity-75 transition-opacity px-1"
                >
                    ...
                    <motion.span
                      ...
                      animate={{
                        ...
                        boxShadow: [
                          ...
                          "0px 0px 10px rgba(var(--color-primary), 0.5)",
                          "0px 0px 0px rgba(0,0,0,0)",
                        ],
                      }}
              ...
              <div className="w-full border border-white/40 bg-white/80 backdrop-blur-md rounded-[2.5rem] overflow-hidden shadow-xl shadow-primary/5">
                ...
                <form onSubmit={handleSave} className="p-8 space-y-8">
                  ...
                  <div id="setup-log-channel">
                    ...
                    />
                    <p className="text-[10px] text-text-secondary mt-2 ml-1 opacity-70">
                      Bot will post moderation logs and AI analysis reports to
                      this channel.
                    </p>
                  </div>
                          ...
                          <div className="bg-white/80 backdrop-blur-md border border-white/40 rounded-[2.5rem] overflow-hidden flex flex-col shadow-xl shadow-primary/5 flex-1 min-h-0">
                            <div className="px-8 py-6 border-b border-outline-variant/20 bg-surface-container/30 flex justify-between items-center shrink-0">
                              <div>
                                ...
                                <p className="text-[10px] font-black text-text-secondary uppercase tracking-widest mt-1">
                                  Configured keywords block messages before
                                  hitting AI.
                                </p>
                              </div>
                            </div>
                                ...
                                <div className="flex items-center">
                                  ...
                                  <div className="ml-4 flex flex-col">
                                    ...
                                    <span className="text-[10px] text-text-secondary mt-1">
                                      ...
                                      contain any active filter keywords,
                                      without invoking extreme threshold AI
                                      reasoning.
                                    </span>
                                  </div>
                                </div>
                                ...
                                <TooltipPrimitive.Root>
                                  ...
                                  <TooltipPrimitive.Portal>
                                    <TooltipPrimitive.Content
                                      ...
                                      align="center"
                                      sideOffset={5}
                                      className="w-64 p-2.5 bg-gray-800 border border-gray-700 text-gray-100 text-[11px] font-semibold rounded-xl shadow-2xl z-[9999] text-center leading-relaxed tracking-wide animate-in fade-in zoom-in-95 data-[state=closed]:animate-out data-[state=closed]:fade-out data-[state=closed]:zoom-out-95 shadow-black/50"
                                    >
                                      Keeping your rules under 15 helps the AI
                                      stay sharp. Too many rules can distract
                                      the bot and dilute the context, making it
                                      less accurate at enforcing what matters
                                      most.
                                      <TooltipPrimitive.Arrow className="fill-gray-800" />
                                    </TooltipPrimitive.Content>
                                  </TooltipPrimitive.Portal>
                              ...
                              {rules.length === 0 ? (
                                <div className="flex flex-col flex-1 items-center justify-center py-12 px-6 text-center h-full">
                                  ...
                                  <p className="text-xs text-text-secondary max-w-[200px] leading-relaxed font-medium">
                                    Add standard rules here to instruct the AI
                                    moderation model on what is strictly
                                    forbidden.
                                  </p>
                                </div>
                              ) : (
                    ...
                    {dnaTab === "suggestions" && (
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        <div className="flex flex-col h-[700px]">
                          ...
                          <div className="bg-white/80 backdrop-blur-md border border-white/40 rounded-[2.5rem] overflow-hidden flex flex-col shadow-xl shadow-primary/5 p-8 h-full">
                            <div className="flex flex-col gap-2 mb-8 shrink-0 h-[160px]">
                              <div className="flex items-center gap-3">
                              ...
                              <p className="text-sm text-text-secondary pl-14 leading-relaxed">
                                Populate your server's legal framework with
                                pre-written, AI-optimized rules that handle
                                sarcasm, banter, and veiled threats perfectly
                                out of the box.
                              </p>
                            </div>
                            <div className="space-y-4 overflow-y-auto pr-2 pb-2 flex-1 min-h-0">
                              {SUGGESTED_RULES.map((rule) => {
                                const isRuleAdded = rules
                                  .map((r) => r.text.trim().toLowerCase())
                                  .includes(rule.text.trim().toLowerCase());
                                  ...
                                  >
                                    <div className="flex items-start justify-between gap-4">
                                      ...
                                      <button
                                        ...
                                        onClick={() =>
                                          addSuggestionRule(rule.id, rule.text)
                                        }
                                        disabled={
                        ...
                        <div className="flex flex-col h-[700px]">
                          ...
                          <div className="bg-white/80 backdrop-blur-md border border-white/40 rounded-[2.5rem] overflow-hidden flex flex-col shadow-xl shadow-primary/5 p-8 h-full">
                            <div className="flex flex-col mb-8 gap-2 shrink-0 h-[160px]">
                              <div className="flex items-center gap-3">
