...
export function hasIndirectContemptShape(text: string): boolean {
  ...
  const praiseFrame =
    /\b(amazing|impressive|brilliant|genius|talent|achievement|award|medal|congratulations|great job|well done|beautiful|wonderful|perfect)\b/i;
  const negativeSituation =
    /\b(confusion|incident|harder|hard|difficult|easiest|easy|basic|simple|obvious|mess|problem|issue|mistake|failure|failed|worse|disaster|chaos|explained more than once|three attempts|again)\b/i;
  ...
  if (vagueTarget.test(normalized) && negativeSituation.test(normalized)) {
    return true;
  }

  if (
    praiseFrame.test(normalized) &&
    negativeSituation.test(normalized) &&
