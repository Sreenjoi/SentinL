              ...
              if (perms && perms.has(PermissionFlagsBits.SendMessages)) {
                ...
                if (perms.has(PermissionFlagsBits.EmbedLinks)) {
                  const embed = new EmbedBuilder()
                    ...
                    .setFooter({
                      text: "AI moderation will resume automatically after reset.",
                    })
                    .setColor(0xffa500);
...
export async function resolveUserReport(
  ...
  try {
    ...
    if (action === "ban" && member) {
      ...
      await member.timeout((duration || 60) * 60 * 1000, reason);
      addBotLog(`[Mod Action] Timed out member ${targetUserId} in ${serverId}`);
    } else if (action === "warn" && member) {
      ...
      if (data.reportedChannelId) {
        try {
          ...
          addBotLog(
            `[Mod Action Error] Failed to delete message ${data.reportedMessageId}: ${e.message}`,
          );
        }
      }
    }
    ...
    for (const sub of activeSubs.docs) {
      ...
      if (noticeType > 0) {
        ...
        const embedOptions = {
          ...
          footer: {
            text: autoRenew
              ? "SentinL Billing (Autopay Enabled)"
              : "SentinL Billing",
          },
        };
    ...
    while (hasMore) {
      ...
      for (const doc of serversSnap.docs) {
        ...
        const embed = new EmbedBuilder()
          ...
          .setFooter({ text: "SentinL Analytics" })
          .setTimestamp();
        ...
        if (interaction.isChatInputCommand()) {
          ...
          if (commandName === "reports") {
            ...
            if (sub === "take") {
              ...
              await reportRef.update({ moderatorId: interaction.user.id });
              return interaction.editReply(
          ...
          if (commandName === "help") {
            ...
            const embed = new EmbedBuilder()
              ...
              .setFooter({
                text: `Full configuration available at ${APP_URL}`,
              });
          ...
          if (commandName === "rank") {
            ...
            const rankEmbed = new EmbedBuilder()
              ...
              .setFooter({
                text: "Generate these stats for your server with SentinL",
              });
...
${escapeForPromptBlock(combinedContent)}
    ...
    const decisionList = messagesToProcess.map((msg) => {
      ...
      if (isSafe && isLowConfidenceLinguisticRisk) {
        ...
        analysis.reason =
          "Possible unfamiliar or cross-language slang under this server's safety rules. Needs moderator review.";
        analysis._nuancedToxicity = true;
        analysis.reviewOnly = true;
        analysis.detectionMethod = "ai_review_only";
        forceReviewOnly = true;
      ...
      const attribution = attributeModerationRule({
        ...
        triggeredLocalSignals: {
          ...
          hasTransliteration: analysis._routedByLinguistic,
          hasSpam,
          hasInvite,
          hasUrl,
...
export async function updateServerHealthWidget(
  ...
  const safeFooterText = healthWidget.motto
    ? `${healthWidget.motto.substring(0, 200)} | Analyzed and Protected by SentinL.app`
    : "Analyzed and Protected by SentinL.app";
          ...
          for (const milestone of newMilestones) {
            ...
            if (mChannel && mChannel.isTextBased()) {
              ...
              const milestoneEmbed = {
                ...
                color: 0xffd700, // Gold
                footer: { text: "Analyzed and Protected by SentinL.app" },
                timestamp: new Date().toISOString(),
              };
