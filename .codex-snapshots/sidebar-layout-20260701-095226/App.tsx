...
function Layout({ children, isCatchAll = false }: { children: React.ReactNode, isCatchAll?: boolean }) {
  ...
  const navItems = [
    ...
    { path: "/moderation", label: "Content Moderation", icon: ListChecks, show: true },
    {
      path: "/settings",
      label: "Bot Settings",
      icon: SettingsIcon,
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      >
        ...
        <div className="flex flex-row justify-center items-center gap-0.5 text-[9px] font-black uppercase tracking-widest opacity-60">
          <span className="px-1 py-0.5 bg-on-surface/5 rounded">⌘ K</span>
        </div>
      </button>
      ...
      <nav className="flex-1 flex flex-col gap-1 overflow-y-auto min-h-0 custom-scrollbar pr-2 pb-2">
        {navItems
