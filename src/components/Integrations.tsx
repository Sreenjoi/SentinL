...
export default function Integrations() {
  const { selectedServerId, tier, user, isBetaTester, isTrial, isPro, isSharedServer } = useServer();
  ...
  return (
    ...
    >
      <BrandedPageHeader
        ...
        description="Automatically announce new YouTube videos and Twitch streams."
        icon={Bell}
        meta={
          <HeaderMetaPills
            planLabel={getPlanDisplayLabel({ tier, isBetaTester, isTrial, isSharedServer })}
            path={["Social Integrations", activeTab === "youtube" ? "YouTube" : "Twitch"]}
          />
        }
        action={
          <ProGate isPro={isPro} featureName="Social Integrations" featureDescription="Add YouTube or Twitch listeners to your server" className="inline-block relative">
            <button
              ...
              disabled={!isPro}
              className={`flex items-center justify-center gap-2 rounded-full px-6 py-3 text-xs font-black uppercase tracking-widest transition-all duration-300 ease-out shadow-lg shrink-0 ${!isPro ? "border border-white/20 bg-white/10 text-white/60 shadow-none" : "bg-white text-primary hover:bg-white/90 active:scale-95"}`}
            >
      ...
      <div className="max-w-full">
        <BranchTabs
          ...
          items={[
            {
              id: "youtube",
              label: <span className="inline-flex items-center gap-2">YouTube{!isPro && <span className="sentinl-pro-badge">{isBetaTester ? "PRO (Beta)" : "PRO"}</span>}</span>,
              icon: Youtube,
            },
            {
              id: "twitch",
              label: <span className="inline-flex items-center gap-2">Twitch{!isPro && <span className="sentinl-pro-badge">{isBetaTester ? "PRO (Beta)" : "PRO"}</span>}</span>,
              icon: Twitch,
            },
          ]}
        />
      ...
      <AnimatePresence>
        {showAddModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
            ...
            >
              <div className="p-8 border-b border-primary/20 flex justify-between items-center bg-primary text-white">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-white/15 rounded-xl flex items-center justify-center text-white border border-white/20">
                ...
                <button
                  onClick={() => setShowAddModal(false)}
                  className="w-10 h-10 flex items-center justify-center text-white/80 hover:text-white transition-colors duration-300 ease-out bg-white/10 rounded-full"
                >
                  ✕
                </button>
              </div>
