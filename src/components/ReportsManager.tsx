...
export default function ReportsManager({
  hideHeader,
  onTabChange,
}: { hideHeader?: boolean; onTabChange?: (tab: "queue" | "history" | "settings" | "analytics") => void } = {}) {
  const { selectedServerId, tier, isBetaTester, loading: serverLoading, botPermissions, isTrial , isPro, discordProfile} = useServer();
  ...
  >(() => {
    const [primary, secondary] = window.location.hash.replace("#", "").split("/");
    const tabHash = primary === "reports" ? secondary : primary;
    if (["queue", "history", "settings", "analytics"].includes(tabHash)) {
      return tabHash as any;
    }
    return "queue";
  });
  ...
  useEffect(() => {
    const [primary, secondary] = location.hash.replace("#", "").split("/");
    const tabHash = primary === "reports" ? secondary : primary;
    if (["queue", "history", "settings", "analytics"].includes(tabHash)) {
      setActiveTab(tabHash as any);
    }
  }, [location.hash]);
  ...
  const [muteDurationMinutes, setMuteDurationMinutes] = useState(60);
  ...
  useEffect(() => {
    setModeratorNote("");
    setMuteDurationMinutes(60);
  }, [selectedReport?.id]);

  const currentUserId = auth.currentUser?.uid || null;
  const getAssigneeLabel = (report: Report) => {
    if (!report.assigneeId) return "Unassigned";
    if (
      (currentUserId && report.assigneeId === currentUserId) ||
      (discordProfile?.id && (report.assigneeId === discordProfile.id || report.assigneeDiscordId === discordProfile.id))
    ) return "You";
    return report.assigneeName || report.assigneeId.substring(0, 10);
  };
  const isAssignedToCurrentUser = (report: Report) =>
    Boolean(
      report.assigneeId &&
        ((currentUserId && report.assigneeId === currentUserId) ||
          (discordProfile?.id && (report.assigneeId === discordProfile.id || report.assigneeDiscordId === discordProfile.id))),
    );
  const isAssignedToOther = (report: Report | null) =>
    Boolean(report?.assigneeId && !isAssignedToCurrentUser(report));

  const handleAssign = async (reportId: string) => {
    if (!selectedServerId) return;
    try {
      const token = await auth.currentUser?.getIdToken();
      const res = await fetch(`/api/guilds/${selectedServerId}/reports/${reportId}/assign`, {
        method: "POST",
        headers: {
      ...
      if (!res.ok) {
        ...
        throw new Error(errData.error || "Failed to assign report");
      }

      toast.success(`Report assigned to ${discordProfile?.username || auth.currentUser?.displayName || "you"}.`);
    } catch (err: any) {
  ...
  const handleDelete = async (reportId: string) => {
    if (!selectedServerId) return;
    if (isAssignedToOther(selectedReport)) {
      toast.error(`This report is assigned to ${getAssigneeLabel(selectedReport!)}. You can view it, but only the assigned moderator can archive it.`);
      return;
    }
  ...
  const handleAction = async (
    ...
    if (!selectedServerId) return;
    if (isAssignedToOther(selectedReport)) {
      toast.error(`This report is assigned to ${getAssigneeLabel(selectedReport!)}. You can view it, but only the assigned moderator can take action.`);
      return;
    }
    ...
    if (action === "delete_message" && selectedReport && (!selectedReport.reportedMessageId || !selectedReport.reportedChannelId)) {
      toast.error("This report is not attached to a specific message, so SentinL cannot delete a message from it.");
      return;
    }
      ...
      const response = await fetch(
        `/api/guilds/${selectedServerId}/reports/${reportId}/resolve`,
        {
          ...
          body: JSON.stringify({
            action,
            reason,
            duration: action === "timeout" ? muteDurationMinutes : undefined,
  ...
  const reportTabItems = [
    { id: "queue" as const, label: "Active Reports", icon: Clock, badge: pendingReportsCount > 0 ? pendingReportsCount : undefined },
    { id: "history" as const, label: "Report History", icon: ListChecks },
    { id: "analytics" as const, label: "Insights", icon: BarChart3 },
    { id: "settings" as const, label: "Settings", icon: Settings },
  ];
  const renderReportTabs = (compactLabels = false) => (
    <BranchTabs
      level="tertiary"
      active={activeTab}
      onChange={(tab) => {
        setActiveTab(tab);
        onTabChange?.(tab);
        window.history.replaceState(null, "", `#reports/${tab}`);
      }}
      items={reportTabItems.map((tab) => ({
        ...tab,
        label: compactLabels && tab.id !== "queue" ? tab.label.replace("Report ", "") : tab.label,
      }))}
    />
  );
    ...
    <div className="space-y-8 pb-32">
      ...
      >
        ...
        {hideHeader && portalNode
          ...
          : !hideHeader && (
              <div className="mt-2">{renderReportTabs(true)}</div>
            )}
      </header>
      ...
      {activeTab === "queue" ? (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
          ...
          >
            <div className="bg-white/50 backdrop-blur-md rounded-[2rem] sm:rounded-[2.5rem] border border-white/60 overflow-hidden shadow-xl shadow-primary/5">
              <div className="p-4 sm:p-5 border-b border-outline-variant/20 bg-white/70 backdrop-blur-xl flex flex-col sm:flex-row gap-4">
                <div className="relative flex-1 group">
                  ...
                  <input
                    ...
                    className="w-full bg-white/80 border border-outline-variant/20 rounded-2xl pl-12 pr-4 py-3 text-sm font-bold focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none transition-all duration-300 ease-out"
                    value={searchQuery}
              ...
              <AnimatePresence>
                {filteredReports.map((report, idx) => (
                  <motion.div
                    ...
                    className="group cursor-pointer rounded-2xl p-5 transition-all duration-300 hover:bg-primary hover:shadow-[0_16px_38px_rgba(255,111,97,0.18)] active:bg-primary"
                  >
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-lg bg-danger-container/20 flex items-center justify-center text-danger border border-danger/10 shadow-inner transition-all duration-300 group-hover:border-white/25 group-hover:bg-white/20 group-hover:text-white">
                          <Flag className="w-4 h-4" />
                        </div>
                        <span className="font-extrabold text-sm text-on-surface tracking-tight transition-colors group-hover:text-white">
                          @{report.reportedUsername || report.reportedUserId}
                        </span>
                      </div>
                      <span
                        className={`px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest border transition-all duration-300 group-hover:border-white/25 group-hover:bg-white/20 group-hover:text-white ${
                    ...
                    <p className="text-xs font-medium text-text-secondary line-clamp-2 mb-3 transition-colors group-hover:text-white/78">
                      {report.reason}
                    </p>
                    <div className={`mb-3 inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[8px] font-black uppercase tracking-widest transition-all duration-300 ${
                      report.assigneeId
                        ? "border-primary/20 bg-primary/10 text-primary group-hover:border-white/25 group-hover:bg-white/20 group-hover:text-white"
                        : "border-outline-variant/25 bg-white/50 text-text-secondary group-hover:border-white/20 group-hover:bg-white/10 group-hover:text-white/72"
                    }`}>
                      <User className="h-3 w-3" />
                      {report.assigneeId ? `Assigned to ${getAssigneeLabel(report)}` : "Unassigned"}
                    </div>
                    <div className="flex items-center justify-between text-[9px] font-black text-text-secondary uppercase tracking-widest transition-colors group-hover:text-white/72">
                      ...
                      <ChevronRight className="w-4 h-4 opacity-40 transition-all group-hover:translate-x-1 group-hover:text-white group-hover:opacity-100" />
                    </div>
                  </motion.div>
                ))}
              </AnimatePresence>
              ...
              <div className={`overflow-x-auto ${selectedReport ? "hidden" : "hidden sm:block"}`}>
                {filteredReports.length === 0 ? (
                  ...
                  <table className="w-full text-left text-sm min-w-[700px] md:min-w-0">
                    <thead className="bg-surface-container/90 backdrop-blur-md text-text-secondary uppercase text-[9px] font-black tracking-[0.2em] border-b border-outline-variant/10 shadow-sm">
                      <tr>
                    ...
                    <tbody className="divide-y divide-outline-variant/10">
                      ...
                      {filteredReports.map((report, idx) => (
                        <motion.tr
                          ...
                          onClick={() => setSelectedReport(report)}
                          className={`cursor-pointer transition-all duration-300 group hover:bg-primary hover:shadow-[0_16px_38px_rgba(255,111,97,0.18)] ${selectedReport?.id === report.id ? "bg-white/80" : ""}`}
                        >
                          <td className="px-6 sm:px-8 py-6 min-w-0">
                            <div className="flex justify-start items-start gap-3">
                              <div className="shrink-0 mt-0.5">
                                <div className="w-9 h-9 rounded-xl bg-danger-container/20 flex items-center justify-center text-danger transition-all duration-300 ease-out shadow-inner border border-danger/10 group-hover:scale-110 group-hover:border-white/25 group-hover:bg-white/20 group-hover:text-white">
                                  <Flag className="w-5 h-5" />
                                </div>
                              </div>
                              <div className="flex flex-col items-start min-w-0">
                                <div className="flex items-center gap-1.5">
                                  <div className="text-on-surface font-bold text-sm tracking-tight truncate max-w-[120px] sm:max-w-none transition-colors group-hover:text-white">
                                    @{report.reportedUsername || report.reportedUserId}
                                  </div>
                                  <button
                                    onClick={(e) => {
                                    ...
                                    title="Copy User ID"
                                    className="text-text-secondary hover:text-primary transition-colors duration-300 ease-out inline-flex shrink-0 group-hover:text-white/75"
                                  >
                                    <ExternalLink className="w-3 h-3" />
                                  </button>
                                </div>
                                <div className="text-[10px] flex flex-col items-start gap-0.5 mt-0.5">
                                  <span className="text-text-secondary font-semibold transition-colors group-hover:text-white/72">
                                    ID: <span className="font-mono">{report.reportedUserId}</span>
                                  </span>
                                  <span
                                    className="text-text-secondary font-medium cursor-help transition-colors group-hover:text-white/78"
                                    title={report.timestamp ? format(new Date(report.timestamp?.seconds * 1000 || report.timestamp), "yyyy-MM-dd HH:mm:ss") : ""}
                                  >
                                    {report.reason}
                                  </span>
                                  <span className={`mt-1 inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[8px] font-black uppercase tracking-widest transition-all duration-300 ${
                                    report.assigneeId
                                      ? "border-primary/20 bg-primary/10 text-primary group-hover:border-white/25 group-hover:bg-white/20 group-hover:text-white"
                                      : "border-outline-variant/25 bg-white/60 text-text-secondary group-hover:border-white/25 group-hover:bg-white/10 group-hover:text-white/72"
                                  }`}>
                                    <User className="h-3 w-3" />
                                    {report.assigneeId ? `Assigned to ${getAssigneeLabel(report)}` : "Unassigned"}
                                  </span>
                          ...
                          <td className="px-6 sm:px-8 py-6 text-center sm:text-left whitespace-nowrap">
                            <span
                              className={`px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest shadow-sm transition-all duration-300 group-hover:border-white/25 group-hover:bg-white/20 group-hover:text-white ${
                                report.status === "pending"
                          ...
                          <td className="px-6 sm:px-8 py-6 text-[11px] font-black text-text-secondary uppercase tracking-widest whitespace-nowrap transition-colors group-hover:text-white/72">
                            <span title={format(new Date(report.timestamp?.seconds * 1000 || report.timestamp), "yyyy-MM-dd HH:mm:ss")}>
                              {formatDistanceToNow(new Date(report.timestamp?.seconds * 1000 || report.timestamp), { addSuffix: true })}
                            </span>
                          </td>
                          <td className="px-6 sm:px-8 py-6 text-right">
                            <ChevronRight className="w-5 h-5 text-text-secondary group-hover:text-white group-hover:translate-x-1 transition-all duration-300 ease-out ml-auto opacity-40 group-hover:opacity-100" />
                          </td>
                        </motion.tr>
                      ))}
                      </AnimatePresence>
              ...
              >
                <div className="p-6 sm:p-8 border-b border-primary/20 flex justify-between items-center bg-primary">
                      ...
                      <p className="text-[9px] font-black text-white/80 uppercase tracking-[0.2em] mt-1 flex items-center gap-2">
                        {selectedReport.status === "pending" ? (
                          selectedReport.assigneeId ? (
                            <span className="inline-flex items-center gap-1.5 rounded-full border border-white/25 bg-white/15 px-2.5 py-1 text-white">
                              <User className="h-3 w-3" />
                  ...
                  <div className="flex gap-2">
                    ...
                    {selectedReport.status === "pending" && selectedReport.assigneeId && (
                      <div className="hidden sm:flex h-10 sm:h-12 items-center rounded-2xl border border-white/20 bg-white/15 px-4 text-[10px] font-black uppercase tracking-widest text-white">
                        {isAssignedToCurrentUser(selectedReport) ? "Assigned to you" : `Assigned to ${getAssigneeLabel(selectedReport)}`}
                      </div>
                    )}
                    <button disabled={isAssignedToOther(selectedReport)} onClick={() => handleDelete(selectedReport.id)} className="w-10 h-10 sm:w-12 sm:h-12 bg-white/20 rounded-2xl border border-white/20 flex items-center justify-center text-white hover:bg-white/30 hover:border-white/40 transition-all duration-300 ease-out active:scale-90 shadow-sm disabled:cursor-not-allowed disabled:opacity-45 disabled:hover:bg-white/20 disabled:hover:border-white/20" title={isAssignedToOther(selectedReport) ? `Assigned to ${getAssigneeLabel(selectedReport)}` : "Archive Report"}>
                ...
                <div className="p-6 sm:p-8 space-y-6 sm:space-y-8">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                    <div className="bg-primary p-4 sm:p-5 rounded-2xl border border-white/20 shadow-lg shadow-primary/15 text-white">
                      <div className="text-[9px] font-black text-white/70 uppercase tracking-widest mb-3 ml-1">
                        Reporter
                      </div>
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-white/15 border border-white/20 flex items-center justify-center text-white shadow-sm">
                          <User className="w-4 h-4" />
                        </div>
                        <div className="flex flex-col min-w-0">
                          <div className="flex items-center gap-1.5">
                            <span className="font-black text-white tracking-tight text-sm truncate max-w-[120px]">
                              @
                            ...
                            <button
                              ...
                              title="Copy User ID"
                              className="text-white/65 hover:text-white transition-colors duration-300 ease-out inline-flex shrink-0"
                            >
                              <ExternalLink className="w-3.5 h-3.5" />
                            </button>
                          </div>
                          <span className="text-[9px] text-white/70 font-medium truncate mt-0.5">
                            UID: <CopyableId id={selectedReport.reporterId} className="text-white/75 hover:!text-white" />
                          </span>
                        </div>
                      </div>
                    </div>
                    <div className="bg-primary p-4 sm:p-5 rounded-2xl border border-white/20 shadow-lg shadow-primary/15 text-white">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col min-w-0">
                          ...
                          <span className="text-[9px] text-white/70 font-medium truncate mt-0.5">
                            UID: <CopyableId id={selectedReport.reportedUserId} className="text-white/75 hover:!text-white" />
                          </span>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="bg-primary p-5 sm:p-6 rounded-3xl border border-white/20 space-y-3 sm:space-y-4 shadow-lg shadow-primary/15 relative overflow-hidden group text-white">
                    <div className="absolute top-0 right-0 p-4 text-white opacity-15 group-hover:opacity-25 transition-opacity duration-300 ease-out">
                      <MessageSquare className="w-10 sm:w-12 h-10 sm:h-12" />
                    </div>
                    <div className="text-[9px] font-black text-white/70 uppercase tracking-widest leading-none">
                      Reason
                    </div>
                    <p className="text-white font-semibold italic leading-relaxed text-xs sm:text-sm relative pr-8">
                  ...
                  {selectedReport.status === "pending" ? (
                    <div className="pt-6 sm:pt-8 border-t border-outline-variant/10">
                      {isAssignedToOther(selectedReport) && (
                        <div className="mb-5 rounded-2xl border border-primary/20 bg-primary/10 px-4 py-3 text-xs font-bold text-primary">
                          Assigned to {getAssigneeLabel(selectedReport)}. You can read this report, but moderation actions are locked unless it is reassigned.
                        </div>
                      )}
                      ...
                      <div className="mb-6">
                        <textarea
                          ...
                          value={moderatorNote}
                          disabled={isAssignedToOther(selectedReport)}
                          onChange={(e) => {
                      ...
                      <div className="mb-6 grid grid-cols-1 sm:grid-cols-[minmax(0,1fr)_180px] gap-4 rounded-2xl border border-white/20 bg-primary p-4 text-white shadow-lg shadow-primary/15">
                        <div>
                          <div className="text-[9px] font-black text-white/70 uppercase tracking-[0.2em] mb-1">
                            Mute Duration
                          </div>
                          <p className="text-[11px] font-semibold text-white/78 leading-relaxed">
                            Used only when you choose Mute. Other actions ignore this value.
                          </p>
                        </div>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setMuteDurationMinutes(
                                Math.min(10080, Math.max(1, Number(e.target.value) || 60)),
                              )
                            }
                            className="w-full rounded-2xl border border-white/25 bg-white px-4 py-3 text-sm font-black text-on-surface shadow-sm outline-none transition-all focus:border-white focus:ring-4 focus:ring-white/20"
                          />
        ...
        <div className="space-y-8">
          {/* Filter Bar */}
          {filteredReports.length > 0 && (
          <div className="bg-white/50 backdrop-blur-md rounded-[2rem] sm:rounded-[2.5rem] border border-white/60 p-4 sm:p-5 shadow-xl shadow-primary/5 flex flex-col sm:flex-row gap-4 items-center justify-between">
          ...
          <div className="space-y-6">
            {filteredReports.length === 0 ? (
              <div className="bg-white/50 backdrop-blur-md rounded-[2rem] sm:rounded-[2.5rem] border border-white/60 p-4 sm:p-6 shadow-xl shadow-primary/5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredReports.map((report) => (
                  <div key={report.id} className="relative bg-primary text-white border border-primary/20 shadow-xl shadow-primary/15 hover:shadow-2xl hover:-translate-y-1 transition-all duration-500 rounded-[2.25rem] p-5 lg:p-6 flex flex-col overflow-hidden group">
                    ...
                    <div className="flex justify-between items-start mb-5">
                      <div className="flex items-center gap-3 relative z-10 w-full">
                        <div className="w-12 h-12 rounded-[1.25rem] flex shrink-0 items-center justify-center border border-white/25 bg-white/15 text-white shadow-inner">
                          {report.status === "dismissed" ? <Ban className="w-5 h-5" /> :
                           report.status === "pending" ? <Clock className="w-5 h-5" /> :
                           <CheckCircle className="w-5 h-5" />}
                        </div>
                        <div className="flex flex-col min-w-0 flex-1">
                          <span className="px-2.5 py-1 w-fit rounded-xl text-[9px] font-black uppercase tracking-[0.2em] shadow-sm border border-white/25 bg-white/15 text-white">
                            {report.status}
                          </span>
                          <div className="text-[10px] font-black text-white/75 uppercase tracking-[0.2em] mt-2 truncate w-full flex items-center gap-1.5" title={format(new Date(report.timestamp?.seconds * 1000 || report.timestamp), "yyyy-MM-dd HH:mm:ss")}>
                    ...
                    <div className="relative z-10 space-y-5 flex-1">
                      <div className="bg-white border border-white/70 rounded-2xl p-4 flex items-center gap-3 relative overflow-hidden shadow-sm">
                        <div className="w-8 h-8 rounded-full bg-primary/10 flex shrink-0 items-center justify-center text-primary">
                          <User className="w-4 h-4" />
                        </div>
                      ...
                      <div className="flex-1 bg-white border border-white/70 rounded-2xl p-4 shadow-sm">
                    ...
                    <div className="mt-6 pt-5 border-t border-white/20 flex items-center justify-between relative z-10">
                      <div className="flex items-center gap-2 text-xs font-bold text-white/80 truncate pr-3">
                        <span className="w-6 h-6 shrink-0 rounded-full bg-white/20 flex items-center justify-center text-white"><User className="w-3 h-3" /></span>
                        <span className="truncate">@{report.reporterUsername || report.reporterId}</span>
                      </div>
                      {(report as any).actionTaken && (
                        <div className="text-[8px] uppercase font-black tracking-widest px-2.5 py-1.5 bg-white/20 rounded-md text-white border border-white/25 shrink-0 shadow-sm whitespace-nowrap">
                          {(report as any).actionTaken}
                        </div>
                      )}
                    </div>
        ...
        <div className="bg-white/80 backdrop-blur-md border border-white/40 rounded-[2rem] sm:rounded-[2.5rem] overflow-hidden shadow-xl shadow-primary/5 w-full">
          <div className="px-6 sm:px-8 py-5 sm:py-6 border-b border-primary/20 bg-primary text-white">
            <div className="flex items-center gap-3">
