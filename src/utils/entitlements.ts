...
export async function getServerTierStatus(serverId: string, dbRef: any): Promise<TierStatus> {
  ...
  const serverLinkSnap = await dbRef.collection("server_subscriptions").doc(serverId).get();
  if (serverLinkSnap.exists) {
    const linkData = serverLinkSnap.data();
    const linkExpiry = getTimestamp(linkData?.expiresAt);
    if ((linkData?.isBeta || linkData?.isBetaTester) && linkExpiry && linkExpiry.toMillis() > Date.now()) {
      const linkSub: TierStatus = {
        isPremium: true,
        tier: linkData.tier || linkData.accessTier || "pro_1",
        status: "active",
        isBeta: true,
        isTrial: false,
        source: "standalone",
        ownerId: linkData.ownerId || null,
        expiry: linkExpiry,
        linkedServerIds: [serverId],
        maxServers: linkData.maxServers || ((linkData.tier === "premium" || linkData.tier === "pro_3") ? 3 : 1)
      };
      premiumTierCache.set(serverId, linkSub);
      return linkSub;
    }
  }
