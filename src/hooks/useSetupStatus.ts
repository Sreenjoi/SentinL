...
export interface SetupStatus {
  ...
  logComplete: boolean;
  serverTypeComplete: boolean;
  serverTypePresetId: string;
...
export function useSetupStatus(): SetupStatus {
  ...
  const [serverTypePresetId, setServerTypePresetId] = useState("");
    ...
    const unsubServer = onSnapshot(
      ...
      (docSnap) => {
        if (docSnap.exists()) {
          ...
          setServerTypePresetId(typeof data.serverTypePresetId === "string" ? data.serverTypePresetId : "");
  ...
  return useMemo(() => {
    ...
    const inviteComplete =
      !!selectedServerId &&
      (!!botPermissions || botGuilds.includes(selectedServerId));
    const activateComplete = isServerActiveGlobally;
    const permissionsComplete =
      !!botPermissions &&
      ["ViewChannel", "SendMessages", "ManageMessages", "ModerateMembers", "ReadMessageHistory", "EmbedLinks"].every(
        (permission) => botPermissions[permission] === true,
      );
    const intentsComplete = !intentsWarning;
    const logComplete = logChannelSet;
    const serverTypeComplete = !!serverTypePresetId;
    ...
    const tasks: SetupStatusTask[] = [
      ...
      { id: "serverType", isComplete: serverTypeComplete },
